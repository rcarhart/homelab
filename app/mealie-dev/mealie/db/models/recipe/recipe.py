...
if TYPE_CHECKING:
    ...
    from ..shared import SharedRecipeLink
...
class RecipeModel(SqlAlchemyBase, BaseMixins):
    ...
    source_recipe_id: Mapped[GUID | None] = mapped_column(GUID, sa.ForeignKey("recipes.id"), index=True)
    source_recipe: Mapped["RecipeModel | None"] = orm.relationship(
        "RecipeModel", remote_side="RecipeModel.id", foreign_keys=[source_recipe_id], uselist=False
    )
    is_household_copy: Mapped[bool] = mapped_column(sa.Boolean, nullable=False, default=False)
    ...
    shared_links: Mapped[list["SharedRecipeLink"]] = orm.relationship(
        "SharedRecipeLink", cascade="all, delete, delete-orphan"
    )
